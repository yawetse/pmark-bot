...
Production trace alerts measure backend server behavior. Scope latency and error-rate queries to `kind_string = 'Server'`; expected outbound client work, including account-identity fallback probes, is not part of the backend server SLO. Require the threshold for the full five-minute evaluation window so one transient data point does not create a firing and resolved notification pair.

Alert issue closure is controlled by a manual workflow run, the latest SigNoz notification, labels, and repository evidence. A `signoz-resolved` issue closes when the manual `close_resolved` operation runs, its newest SigNoz status is still `resolved`, and no open remediation pull request references the issue. The workflow rechecks the newest status immediately before closing because the resolved label can remain after an alert fires again. A synthetic or smoke-test alert closes when the manual triage operation runs and Codex produces no remediation patch. Other firing production alerts stay open until they are resolved, fixed, or manually closed.
