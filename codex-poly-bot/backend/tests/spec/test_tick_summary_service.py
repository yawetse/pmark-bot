...
class AlwaysFailSummaryTransport:
    def __init__(self) -> None:
        self.calls: list[dict[str, Any]] = []
...
def test_req_obs_005_tick_summary_retries_with_low_cost_fallback_model(monkeypatch) -> None:
    ...
    Then: the summary retries with the fallback and records provider usage without an APM error span
    """
    ...
    assert spans[0]["attributes"]["status"] == "retrying"
    assert spans[0]["attributes"]["error_type"] == "RuntimeError"
    assert spans[0]["attributes"]["message"] == "model not available"
    assert spans[1]["attributes"]["model"] == "gpt-5-mini"
    assert spans[1]["attributes"]["status"] == "success"
    assert recorded_failures == []


def test_req_obs_005_tick_summary_records_apm_failure_after_fallbacks_exhausted(monkeypatch) -> None:
    """TST-REQ-OBS-005-12: Validates REQ-OBS-005

    Given: every configured OpenAI tick summary model fails
    When: no fallback model succeeds
    Then: the final failed attempt is recorded as an APM failure
    """
    ...
    transport = AlwaysFailSummaryTransport()
    service = TickSummaryService(
        registry=RepositoryRegistry(),
        environ={
            "OPENAI_API_KEY": "test-key",
