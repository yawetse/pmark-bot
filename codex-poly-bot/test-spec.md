...
| TST-REQ-KAL-007-03 | Fresh Control State | REQ-KAL-007 | Given persisted configuration or kill-switch state changes while a scheduled tick is running, When the tick reaches scanner, scoring, and execution stage boundaries, Then it rereads current state, excludes Kalshi from new scans and scores when killed, and sends zero new exposure mutations while preserving risk-reducing paths. |
