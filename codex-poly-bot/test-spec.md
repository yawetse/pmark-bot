...
| TST-REQ-KAL-007-03 | Fresh Kill Switch | REQ-KAL-007 | Given the persisted kill switch changes while a scheduled tick is running, When the tick reaches scanner, scoring, strategy, and execution stage boundaries, Then it rereads the kill switch, retains the loop's normal config snapshot, excludes Kalshi from new scans, scores, and strategy decisions when killed, and sends zero new exposure mutations while preserving risk-reducing paths. |
...
| TST-REQ-KAL-010-02 | Release | REQ-KAL-010 | Given merged development and production releases, When evidence collection runs, Then commit, workflows, stacks, ECS task and digest, sanitized host and refs, health, OAuth, SES, ACM, screenshots, public exchange status, runtime missing-secret refusal, and rollback gates are recorded; all six credentials additionally require deployed runtime injection, authenticated batch-order-book success, and normalized OpenAI and Claude balances, no credentials require an explicit not-configured result and blocked runtime readiness, and a partial set fails verification. |
...
| TST-REQ-KAL-010-04 | Credential Modes | REQ-KAL-010 | Given zero, partial, and complete six-secret inventories plus sanitized deployed runtime rows, When release credential classification runs, Then zero is explicitly not configured, partial fails, complete succeeds only when all three runtime rows report configured injection, and no secret value is returned. |
...
| REQ-KAL-007 | TST-REQ-KAL-007-01, TST-REQ-KAL-007-02, TST-REQ-KAL-007-03 |
| REQ-KAL-008 | TST-REQ-KAL-008-01, TST-REQ-KAL-008-02 |
| REQ-KAL-009 | TST-REQ-KAL-009-01, TST-REQ-KAL-009-02 |
| REQ-KAL-010 | TST-REQ-KAL-010-01, TST-REQ-KAL-010-02, TST-REQ-KAL-010-03, TST-REQ-KAL-010-04 |
