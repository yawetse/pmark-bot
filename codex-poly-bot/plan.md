...
| 4 | Backend API and dashboard | Backend app, API routers, Frontend app, Auth UI, Dashboard UI, Frontend API client | Phases 1-3 | Local dashboard signs in, shows status, edits config, triggers kill switch, and displays Claude/OpenAI dry-run comparison |
...
| M4: Local dashboard | Open dashboard locally | GitHub auth gate, config editing, kill switch, wallet status, worker health, and comparison views work against local API |
...
|------|--------|------------|-----------------|
| Live trading with incorrect config | High | Seed dry-run and disabled venues, implement refusal matrix and audit before any live adapter submit | Phase 1 and Phase 3 |
